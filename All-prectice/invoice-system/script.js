...
$('resetBtn').onclick = () => confirm("Clear?") && (items = [], $('itemForm').reset(), render());
    ...
    items.forEach(i => {
        ...
        $('invoiceItemsBody').insertAdjacentHTML('beforeend', `
            <tr class="item-row">
                ...
                <td class="col-actions"><button class="btn-icon" onclick="remove(${i.id})"><i class="ri-delete-bin-line"></i></button></td>
            </tr>
        `);
    });

    const tax = (subtotal - totalDisc) * ((+$('globalTax').value || 0) / 100);

    $('subtotalDisplay').textContent = fmt(subtotal);
    $('discountDisplay').textContent = `-${fmt(totalDisc)}`;
    $('taxRateDisplay').textContent = `${+$('globalTax').value || 0}%`;
    $('taxDisplay').textContent = fmt(tax);
    $('grandTotalDisplay').textContent = fmt(subtotal - totalDisc + tax);
}
